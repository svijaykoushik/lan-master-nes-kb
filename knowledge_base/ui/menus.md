...
## 1. Main Menu ([mainmenu.asm](mainmenu.md))
...
- **Features**:
    ...
    - **[Password System](../logic/passwords.md)**: Allows users to enter a 4-digit code. The code is verified against the `passwords` table in `mainmenu.asm`. If valid, `GAME_LEVEL` is set to the corresponding level.
    - **Visuals**: Includes a fade-in effect using `palFadeIn` and a blinking cursor.

## 2. Game Menu ([gamemenu.asm](gamemenu.md))
...
## 3. Well Done Screen ([welldone.asm](welldone.md))
